...
final getUserIssuesUsecaseProvider = Provider<GetUserIssuesUsecase>((ref) {
  return GetUserIssuesUsecase(
    issueRepository: ref.read(issuesRepositoryProvider),
  );
});

class GetUserIssuesUsecase implements UsecaseWithoutParams<List<IssuesEntity>> {
  final IIssueRepository _issueRepository;

  GetUserIssuesUsecase({required IIssueRepository issueRepository})
    : _issueRepository = issueRepository;

  @override
  Future<Either<Failure, List<IssuesEntity>>> call() {
    return _issueRepository.getUserIssues();
  }
}
